...
the 2B model. The model emits one shell command directly, with no JSON step
...
functional, a small and transparent loop with a clean tool interface and no heavy
...
repeatable head, which is precisely the asymmetry the two-tier split exploits.
...
and never edited after observing failures, since editing it would leak the benchmark
...
however much effort the teacher spends producing the pattern. It is the analogue
of a practiced human reflex, refined slowly through feedback yet executed
instantly and without deliberation.
